...
function calcPValueMC(rows,sims){var n=rows.length;if(n<30)return 1;sims=sims||(n>5000?4000:10000);var lucro=new Float64Array(n),stake=new Float64Array(n),sumL=0,sumS=0;for(var i=0;i<n;i++){lucro[i]=+rows[i].lucro||0;stake[i]=+rows[i].stake||0;sumL+=lucro[i];sumS+=stake[i];}if(sumS<=0)return 1;var yieldObs=sumL/sumS,r0=new Float64Array(n);for(var j=0;j<n;j++)r0[j]=lucro[j]-yieldObs*stake[j];var seed=((n*2654435761)^(Math.round(Math.abs(sumL)*1000)|0))>>>0,rng=mulberry32(seed),cnt=0;for(var s=0;s<sims;s++){var rs=0,ss=0;for(var b=0;b<n;b++){var k=(rng()*n)|0;rs+=r0[k];ss+=stake[k];}if(ss>0&&rs/ss>=yieldObs)cnt++;}return(cnt+1)/(sims+1);}
